...
object SFCrimeClassifierNaiveBayes {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val naiveBayesModel = new NaiveBayes().setLabelCol(colLabel + "Indexed").setFeaturesCol(colFeatures).setSmoothing(1.0).
    ...
    val pipeline = new Pipeline().setStages((featureIndex :+ classIndex :+ nbfeatureVector :+ naiveBayesModel :+ indexToString).toArray)
    ...
    print("Naive Classifier Model training initiated..." + "\n")
    ...
    print("Naive Bayes classifier classifying test data" + "\n")
    ...
    predictions.coalesce(3).write.format("csv").option("header", "true").save(nbOutputFile)
  }
