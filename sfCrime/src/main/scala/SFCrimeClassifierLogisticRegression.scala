...
object SFCrimeClassifierLogisticRegression {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val nbfeatureVector = new VectorAssembler().setInputCols((categoryFeatures.map(_ + "Indexed")).toArray).setOutputCol(colFeatures)

    val logisticRegressionModel = new LogisticRegression().setMaxIter(100).setRegParam(0.3).setElasticNetParam(0.8).
      setLabelCol(colLabel + "Indexed").setFeaturesCol(colFeatures)
    ...
    //    val pipeline = new Pipeline().setStages((featureIndex :+ classIndex :+ featureVector :+ decisionTreeModel :+ indexToString).toArray)
    val pipeline = new Pipeline().setStages((featureIndex :+ classIndex :+ featureVector :+ logisticRegressionModel :+ indexToString).toArray)
    ...
    // val parameterGrid = new ParamGridBuilder().addGrid(decisionTreeModel.impurity, Array("entropy", "gini")).build()
    ...
    print("Logistic Regression Classifier Model training initiated..." + "\n")

    // model training initiation
    val cvModel = crossValidator.fit(enrichedTrainDF)

    print("Logisitic Regression classifier training completed..." + "\n")

    // predicting the test dataframe

    print("Logisitic Regression classifier classifying test data" + "\n")
    ...
    print("\n Logistic Regression Classifier Accuracy " + accuracy + "\n")

    predictions.coalesce(3).write.format("csv").option("header", "true").save(lgOutputFile)
  }
